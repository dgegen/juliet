...
def init_batman(t, ld_law, nresampling=None, etresampling=None):
    ...
    if nresampling is None or etresampling is None:
        m = [
            batman.TransitModel(params, t),
            batman.TransitModel(params, t, transittype="secondary"),
        ]

    else:
        m = [
            batman.TransitModel(
                params, t, supersample_factor=nresampling, exp_time=etresampling
            ),
            batman.TransitModel(
                params,
                t,
...
def correct_light_travel_time(times, params):
    ...
    delta_t = (delta_x / const.c.value) / (3600.0 * 24.0)
...
def get_TESS_data(filename, fluxtype="PDCSAP_FLUX"):
    ...
    return (
        data["TIME"][idx],
        data[fluxtype][idx] / np.median(data[fluxtype][idx]),
        data[fluxtype + "_ERR"][idx] / np.median(data[fluxtype][idx]),
    )


def get_all_TESS_data(
    object_name,
    radius=".02 deg",
    get_PDC=True,
    get_all=False,
        ...
        if "s_lc.fits" in obs_table["dataURL"][i]:
            ...
            if len(metadata) == 5:
                ...
                if get_lightcurves_only:
                    ...
                    want = (data_products["description"] == "Light curves") or (
                        data_products["description"] == "Target pixel files"
                    )
                manifest = Observations.download_products(data_products[want])
                # Read lightcurve file:
                d, h = fits.getdata(
                    "mastDownload/TESS/" + fname[:-8] + "/" + fname, header=True
                )
                t, fs, fserr, f, ferr = (
                    d["TIME"] + h["BJDREFI"],
                    d["SAP_FLUX"],
...
def reverse_bp(r1, r2, pl, pu):
    ...
    for i in range(nsamples):
        if r1[i] > Ar:
            b[i], p[i] = (
                (1 + pl) * (1.0 + (r1[i] - 1.0) / (1.0 - Ar)),
                (1 - r2[i]) * pl + r2[i] * pu,
            )
        else:
            b[i], p[i] = (
                (1.0 + pl) + np.sqrt(r1[i] / Ar) * r2[i] * (pu - pl),
                pu + (pl - pu) * np.sqrt(r1[i] / Ar) * (1.0 - r2[i]),
            )
    return b, p
...
def input_error_catcher(t, y, yerr, datatype):
    ...
    if y is None:
        raise Exception(
            "INPUT ERROR: No "
            + datatype
            + " data was fed to juliet. \n"
            + " Make sure to pass data (y_"
            + dname
            + ") and errors (yerr_"
            + dname
            + ")."
    ...
    if yerr is None:
        raise Exception(
            "INPUT ERROR: No ERRORS (yerr_"
            + dname
            + ") on the "
            + datatype
            + " data were fed to juliet. \n"
            + " Make sure to pass data (y_"
            + dname
            + ") and errors (yerr_"
            + dname
        ...
        if line != "":
            ...
            if lm_boolean[instrument.split()[0]]:
                if len(lm_arguments[instrument.split()[0]]) == 0:
                    lm_arguments[instrument.split()[0]] = np.array(lm_variables).astype(
                        np.double
                    )
                else:
                    lm_arguments[instrument.split()[0]] = np.vstack(
                        (
                            lm_arguments[instrument.split()[0]],
...
def readpriors(priorname):
    ...
    if not input_dict:
        ...
        return (
            ...
            n_transit,
            n_rv,
            numbering_transit.astype("int"),
            numbering_rv.astype("int"),
            n_params,
            starting_point,
        )
    ...
    for i in range(0, len(x), n_bin):
        if method == "median":
            ...
            raise Exception(
                'Error: method "'
                + method
                + '" not recognized --- needs to be "mean" or "median".'
            )
        ...
        if yerr is None:
            ...
            y_err_bins.append(
                np.sqrt(np.sum(yerr[i : i + n_bin - 1] ** 2))
                / float(len(yerr[i : i + n_bin - 1]))
            )
    ...
    for pname in posteriors["posterior_samples"].keys():
        if pname != "unnamed" and pname != "loglike":
            ...
            if pname.split("_")[0] == "r2":
                ...
                if "ecosomega_" + planet in posteriors["posterior_samples"]:
                    ...
                    ecc = np.sqrt(
                        ...
                        ** 2
                        + posteriors["posterior_samples"]["esinomega_p" + str(iplanet)]
                        ** 2
                    )
                    omega = np.arctan2(
                        posteriors["posterior_samples"]["esinomega_p" + str(iplanet)],
                    ...
                    ecc = (
                        ...
                        ** 2
                        + posteriors["posterior_samples"]["sesinomega_p" + str(iplanet)]
                        ** 2
                    )
                    omega = np.arctan2(
                        posteriors["posterior_samples"]["sesinomega_p" + str(iplanet)],
                    ...
                    "ecc_" + planet in posteriors["posterior_samples"]
                    and "omega_" + planet in posteriors["posterior_samples"]
                ):
                    ecc = posteriors["posterior_samples"]["ecc_" + planet]
                    omega = (
                        posteriors["posterior_samples"]["omega_" + planet]
                        * np.pi
                        / 180.0
                    )
                ...
                if "rho" in posteriors["posterior_samples"]:
                    ...
                    if "P_" + planet in posteriors["posterior_samples"]:
                        a = (
                            (
                                posteriors["posterior_samples"]["rho"]
                                * G
                                * (
                                    (
                        ...
                        a = (
                            (
                                ...
                                * (
                                    (
                                        priors["P_" + planet]["hyperparameters"]
                                        * 24.0
                                        * 3600.0
                                    )
                                    ** 2
                ...
                elif ("m_star" in posteriors["posterior_samples"]) and (
                    "r_star" in posteriors["posterior_samples"]
                ):
                    # Compute rho from stellar mass and radius in posteriors, then get a
                    G = 6.67408e-11
                    rho_star = (
                        posteriors["posterior_samples"]["m_star"] * const.M_sun.value
                    ) / (
                        (4.0 / 3.0)
                        * np.pi
                        * (
                            posteriors["posterior_samples"]["r_star"]
                            * const.R_sun.value
                        )
                        ** 3
                    )
            ...
            if pname.split("_")[0] == "ecosomega":
                ...
                iplanet = planet[1:]
                ecc = np.sqrt(
                    posteriors["posterior_samples"]["ecosomega_p" + str(iplanet)] ** 2
                    + posteriors["posterior_samples"]["esinomega_p" + str(iplanet)] ** 2
                )
                omega = np.arctan2(
                    posteriors["posterior_samples"]["esinomega_p" + str(iplanet)],
                    posteriors["posterior_samples"]["ecosomega_p" + str(iplanet)],
                ...
                fout.write(
                    "{0:18} \t \t {1:.10f} \t \t {2:.10f} \t \t {3:.10f}\n".format(
                        "omega_" + planet, val, usigma, dsigma
                    )
                )
            ...
            if pname.split("_")[0] == "secosomega":
                ...
                iplanet = planet[1:]
                ecc = (
                    posteriors["posterior_samples"]["secosomega_p" + str(iplanet)] ** 2
                    + posteriors["posterior_samples"]["sesinomega_p" + str(iplanet)]
                    ** 2
                )
                omega = np.arctan2(
